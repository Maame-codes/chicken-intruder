...
import { 
  StyleSheet, Text, View, Dimensions, TouchableOpacity, StatusBar, Image, Modal 
} from 'react-native';
...
export default function ChickenIntruderGame() {
  ...
  // --- CONTROLS (Updated for Mouse Support) ---
  const handleTouchMove = (e) => {
    if (gameState !== 'PLAYING' && gameState !== 'BOSS_FIGHT') return;
    
    // React Native Web maps mouse events to nativeEvent.pageX too!
    shipPos.current = { 
  ...
  const updateGameLogic = () => {
    ...
    chickens.current = chickens.current.map(c => ({
      ...c, y: c.y + (2 + level*0.2), x: c.x + Math.sin(c.y / 30) * 4
  ...
  const renderMainMenu = () => (
    <View style={styles.overlay}>
      ...
      <TouchableOpacity onPress={resetGame} style={styles.btnPrimary}>
    ...
    >
      ...
      {gameState === 'GAME_OVER' && (
        <View style={styles.overlay}>
          ...
          <TouchableOpacity onPress={resetGame} style={styles.btnPrimary}><Text style={styles.btnText}>RETRY</Text></TouchableOpacity>
...
const styles = StyleSheet.create({
  container: { flex: 1, backgroundColor: '#000015' },
