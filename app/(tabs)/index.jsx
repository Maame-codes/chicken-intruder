...
const SHOOT_URL =
  "https://codeskulptor-demos.commondatastorage.googleapis.com/GalaxyInvaders/bonus.mp3";

// --- 🎨 ASSETS ---
const getShipSprite = (id) =>
  `https://robohash.org/${id}.png?set=set3&size=${SHIP_SIZE}x${SHIP_SIZE}`;
const getChickenSprite = (seed) =>
  `https://robohash.org/chicken_${seed}.png?set=set2&size=${CHICKEN_SIZE}x${CHICKEN_SIZE}`;
const getBossSprite = (lvl) =>
  `https://robohash.org/BOSS_INTRUDER_MK${lvl}.png?set=set1&size=${BOSS_SIZE}x${BOSS_SIZE}`;
const getPowerupSprite = () =>
  `https://robohash.org/WEAPON_CRATE.png?set=set4&size=${POWERUP_SIZE}x${POWERUP_SIZE}`;
...
export default function ChickenIntruderGame() {
  ...
  // --- 🖐️ PAN RESPONDER (FIXES MOBILE WEB DRAGGING) ---
    ...
    PanResponder.create({
      ...
      onPanResponderGrant: (evt, gestureState) => {
        ...
        if (gameState === "PLAYING" || gameState === "BOSS_FIGHT") {
          shipPos.current = {
            x: evt.nativeEvent.pageX - SHIP_SIZE / 2,
            y: evt.nativeEvent.pageY - SHIP_SIZE,
          };
        }
      },
  ...
  useEffect(() => {
    async function setupAudio() {
      try {
        ...
        // Load Music (But DO NOT play yet)
  ...
  const playShootSound = async () => {
    try {
      if (sfxShoot) await sfxShoot.replayAsync();
    } catch (e) {}
  };
  ...
  const updateGameLogic = () => {
    ...
    if (gameState === "BOSS_FIGHT" && boss.current) {
      ...
      if (boss.current.x <= 0 || boss.current.x >= SCREEN_WIDTH - BOSS_SIZE)
        boss.current.vx *= -1;
    ...
    pBullets.current = pBullets.current
      .map((b) => ({ ...b, y: b.y - 15 }))
      .filter((b) => b.y > -20);
    bBullets.current = bBullets.current
      .map((b) => ({ ...b, y: b.y + 8 }))
      .filter((b) => b.y < SCREEN_HEIGHT);
    chickens.current = chickens.current
      .map((c) => ({
        ...c,
        y: c.y + (2 + level * 0.2),
        x: c.x + Math.sin(c.y / 30) * 4,
      }))
  ...
  const handleKill = (x, y) => {
    ...
    killCount.current += 1;
    if (Math.random() < 0.1) powerUps.current.push({ id: Math.random(), x, y });
  };
    ...
    <View style={styles.overlay}>
      ...
      <View style={styles.grid}>
        {SKINS.map((s) => (
          <TouchableOpacity
            key={s.id}
            onPress={() => setSelectedSkin(s.id)}
            style={[styles.card, selectedSkin === s.id && styles.cardActive]}
          >
            <Image
              source={{ uri: getShipSprite(s.id) }}
  ...
  return (
    <View
      ...
      {...panResponder.panHandlers} // <--- THIS FIXES THE MOBILE WEB DRAGGING
    >
      <StatusBar hidden />
      ...
      {(gameState === "PLAYING" ||
        ...
        <>
            ...
            <View style={styles.bossHud}>
              ...
              <View style={styles.bossHpBg}>
                <View
                  style={[
                    styles.bossHpFill,
                    { width: `${(Math.max(0, bossHp) / maxBossHp) * 100}%` },
                  ]}
                />
              </View>
            </View>
          ...
          {renderPBullets.map((b) => (
            <View key={b.id} style={[styles.pBull, { left: b.x, top: b.y }]} />
          ))}
          {renderBBullets.map((b) => (
            <View key={b.id} style={[styles.bBull, { left: b.x, top: b.y }]} />
          ))}
          {renderChickens.map((c) => (
            <Image
              key={c.id}
              source={{ uri: getChickenSprite(c.spriteId) }}
              style={[
                styles.ent,
                {
                  left: c.x,
          ...
          {renderPowerUps.map((p) => (
            <Image
              key={p.id}
              source={{ uri: getPowerupSprite() }}
              style={[
                styles.ent,
                {
                  left: p.x,
          ...
          {renderBoss && (
            <Image
              source={{ uri: renderBoss.spriteUrl }}
              style={[
                styles.ent,
                {
                  left: renderBoss.x,
          ...
          <Image
            ...
            style={[
              styles.ent,
              {
                left: shipPos.current.x,
                top: shipPos.current.y,
                width: SHIP_SIZE,
                height: SHIP_SIZE,
              },
            ]}
...
const styles = StyleSheet.create({
  container: { flex: 1, backgroundColor: "#000015", overflow: "hidden" }, // Added overflow:hidden
  overlay: {
  ...
  bBull: {
    position: "absolute",
    width: B_BULLET_SIZE,
    height: B_BULLET_SIZE,
    backgroundColor: "#bf616a",
    borderRadius: B_BULLET_SIZE / 2,
    borderWidth: 1,
    borderColor: "#fff",
  },
